...
However, there are two steps that aren't automated by CI which is GPG signing and macOS binary code-signing.

### Apple signing and notarisation

The seL4 Foundation has an Apple developer account for doing this process.

The Apple documentation can be somewhat lackluster in this regard for CLI tools, in the past
[this blog post](dennisbabkin.com/blog/?t=how-to-get-certificate-code-sign-notarize-macos-binaries-outside-apple-app-store#code_sign)
provides many useful details for setting up the developer certificate.

There are two scripts `release.sh` and `macos_sign.sh` that are relevant.
